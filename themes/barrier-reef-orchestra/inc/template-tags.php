...
function barrier_reef_orchestra_posted_on() {
	...
	//Display avatar if the avatar has a Gravatar
	$author_id = get_the_author_meta('ID');
	if(barrier_reef_orchestra_validate_gravatar($author_id))
	{
...
if ( ! function_exists( 'barrier_reef_orchestra_index_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for post-date/time and author on index pages.
	 */
	function barrier_reef_orchestra_index_posted_on() {
		...
		$posted_on = sprintf(
			esc_html_x( 'Published %s', 'post date', 'barrier_reef_orchestra' ),
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);
		$byline = sprintf(
			esc_html_x( 'by %s', 'post author', 'barrier_reef_orchestra' ),
			'<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>'
		);
		...
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';
			comments_popup_link( esc_html__( 'Leave a comment', 'barrier_reef_orchestra' ), esc_html__( '1 Comment', 'barrier_reef_orchestra' ), esc_html__( '% Comments', 'barrier_reef_orchestra' ) );
			echo '</span>';
		}
		echo '</div><!-- .meta-content -->';
	}
...
function barrier_reef_orchestra_validate_gravatar($id_or_email) {
